...
export function AutonomousResearchPage() {
  ...
  async function openRun(runId: string, nextView: View = "brief") {
    ...
    window.history.replaceState({}, "", `/research?runId=${encodeURIComponent(runId)}`);
    setRun(loadedRun);
    setView(nextView);
  ...
  async function onStarted(nextRun: IntelligenceRun) {
    ...
    window.history.replaceState({}, "", `/research?runId=${encodeURIComponent(nextRun.id)}`);
    await loadRuns();
  }
    ...
    <section className="research-workspace" aria-busy={loading}>
      <header className="research-topbar">
        ...
        <div className="workspace-nav" aria-label="Research navigation">
          ...
          {run ? <button onClick={() => { window.localStorage.removeItem(activeRunStorageKey); window.history.replaceState({}, "", "/research"); setRun(undefined); setBrief(undefined); setLedger(undefined); setView("brief"); }}>New research</button> : null}
        </div>
      </header>
